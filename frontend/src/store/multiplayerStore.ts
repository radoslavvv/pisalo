...
>((set, get) => {
	const setupListeners = () => {
		...
		signalRService.on("error", (error) => {
			set({ error: error as string | null });
		});

		signalRService.on("playerJoined", (player) => {
			set({ opponent: player as Player | null });
		});

		signalRService.on("joinFailed", (reason) => {
			set({ error: reason as string | null, roomStatus: "idle" });
		});
		...
		signalRService.on("gameStarted", (response) => {
			const { words, startTime } = response as GameStartedResponse;
			const typedCharacters: CharacterState[][] = words.map(
				(word: string) => Array(word.length).fill("pending"),
			);
			set({
				roomStatus: "playing",
				words,
		...
		signalRService.on("opponentProgress", (progress) => {
			set({ opponentProgress: progress as PlayerProgress | null });
		});

		signalRService.on("opponentFinished", () => {
			set({
				opponentProgress: {
		...
		signalRService.on("gameEnded", (result) => {
			set({
				roomStatus: "finished",
				gameResult: result as GameEndedResult | null,
			});
		});
	};
	...
	return {
		...
		createRoom: async () => {
			try {
				...
				set({
					...
					host: response.host,
					currentUserId: response.host.id,
					isHost: true,
					roomStatus: "waiting",
					error: null,
				});
		...
		joinRoom: async (roomCode: string) => {
			try {
				const response = await signalRService.joinRoom(
				...
				if (response) {
					set({
						...
						currentUserId: response.guest.id,
						isHost: false,
						roomStatus: "waiting",
						error: null,
					});
		...
		handleKeyPress: (key: string) => {
			...
			if (key === "Backspace") {
				if (currentCharIndex > 0) {
					...
					newTypedCharacters[currentWordIndex] = [
						...newTypedCharacters[currentWordIndex],
					];
					newTypedCharacters[currentWordIndex][currentCharIndex - 1] =
						"pending";
			...
			if (key.length === 1) {
				...
				newTypedCharacters[currentWordIndex][currentCharIndex] =
		...
		sendProgress: async () => {
			...
			const wpm =
				minutes > 0 ? Math.round(state.currentWordIndex / minutes) : 0;
